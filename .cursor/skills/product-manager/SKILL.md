...
5. **Verifier:** `agent` (default) | `ceo` | `n/a`. **Verify passes:** `pass1+pass2` | `pass1` | `pass2` | `n/a`. When Verifier = `ceo`, defaults are Ship path `direct-to-main` + Verify passes `pass2`; **no** agent QA handoffs. **`agent-os-*` always uses `n/a` / `n/a`**; default Ship path **`direct-to-main`** except **self-evolve** (required `feature-branch`) or **CEO-explicit** umbrella PR; never leave `tbd`
...
9. Small doc/protocol updates: PM may execute; for ordinary `agent-os-*`, ship when CEO says **`verified`** (or earlier if CEO explicitly asks to commit/push/merge)
